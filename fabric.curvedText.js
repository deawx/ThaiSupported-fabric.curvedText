(function (global) {
	...
	setText: function (text) {
		...
		for (var i = 0; i < text.length; i++) {
		    ...
		    if (this.except.indexOf(text[i]) > -1) {
			    toInsert += text[i];
			    skip++;
			    continue;
		    }else{
			if(toInsert){
			    this.letters.add(new fabric.Text(toInsert));
			}
			toInsert = text[i];
		    }
		}
	...
	_render: function (ctx) {
		...
		if (this.effect === 'curved') {
			...
			console.log(this.letters.item(i), this.letters.item(i).width);
			textWidth += this.letters.item(i).width + space;
		    }
		    textWidth -= space;
		    console.log(this.letters.size(), textWidth);
		    window.a = this.letters;
		    */
		    console.log(this.fontSize)
		    textWidth = this.letters.size()*(this.fontSize*this.lineheight);
		} else if (this.effect === 'arc') {
		    fixedLetterAngle = ((this.letters.item(0).fontSize + space) / this.radius) / (Math.PI / 180);
		    textWidth = ((this.letters.size() + 1) * (this.letters.item(0).fontSize + space));
		}
		// Text align
		...
		for (var i = 0, len = this.letters.size(); i < len; i++) {
			...
			var center = Math.ceil(this.letters.size() / 2);
			var step = difference / (this.letters.size());
			...
			var step = difference / (this.letters.size());
			var newfont = small + (i * step);
			//var newfont=((this.letters.size()-i)*this.smallFont)+12;
			this.letters.item(i).set('fontSize', (newfont));
			this.letters.item(i).set('left', (width));
			width += this.letters.item(i).get('width');
			this.letters.item(i).set('padding', 0);
			...
			this.letters.item(i).set('padding', 0);
			this.letters.item(i).set('selectable', false);
			this.letters.item(i).top = -1 * this.letters.item(i).get('fontSize') + (i / this.letters.size());
			...
			this.letters.item(i).set('padding', 0);
			this.letters.item(i).set('selectable', false);
			//this.letters.item(i).top =-1* this.letters.item(i).get('fontSize')+newfont-((this.letters.size()-i))-((this.letters.size()-i));
			...
			var difference = large - small;
			var center = Math.ceil(this.letters.size() / 2);
			var step = difference / (this.letters.size() - center);
			if (i < center)
			    var newfont = small + (i * step);
			else
	...
	_renderOld: function (ctx) {
		...
		if (this.get('textAlign') === 'center' || this.get('textAlign') === 'justify') {
		    align = (this.spacing / 2) * (this.letters.size() - rev); // Remove '-1' after this.letters.size() for proper angle rendering
		} else if (this.get('textAlign') === 'right') {
		    align = (this.spacing) * (this.letters.size() - rev); // Remove '-1' after this.letters.size() for proper angle rendering
		}
